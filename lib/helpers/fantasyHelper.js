...
exports.processUserMessage = async function (message, userId) {
    if (message === 'score') {
        ...
        if (teamId === undefined || !Number.isInteger(parseInt(teamId)))
            return 'Please provide fantasy ID in the format "login <Numeric ID>"';
    ...
    return new Promise(function (resolve, reject) {
        ...
        request(options, function (err, res, body) {
            if (err)
                ...
                return resolve('Your fantasy team ID is invalid. Please login again using a valid ID.');
            resolve(body);
        });
    });
}
...
async function getTotalPoints(teamId) {
    ...
    if (typeof teamData === 'string')
        return teamData;
    return 'Current Score - ' + teamData.entry.summary_event_points;
}
